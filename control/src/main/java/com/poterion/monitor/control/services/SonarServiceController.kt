 ...
 */
class SonarServiceController(config: SonarConfig) : ServiceController<SonarConfig>(config) {
    companion object {
    ...
    override fun check(updater: (Collection<StatusItem>) -> Unit) {
        service.check().enqueue(object : Callback<Collection<SonarProjectResponse>> {
            ...
            override fun onFailure(call: Call<Collection<SonarProjectResponse>>?, response: Throwable?) {
                call?.request()?.also { JenkinsServiceController.LOGGER.warn("${it.method()} ${it.url()}: ${response?.message}", response) }
                        ?: JenkinsServiceController.LOGGER.warn(response?.message, response)
                lastFoundProjectNames
                        .mapNotNull { projects[it] }
