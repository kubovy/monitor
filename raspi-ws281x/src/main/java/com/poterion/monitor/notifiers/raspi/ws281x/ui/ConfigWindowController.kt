...
import com.poterion.monitor.notifiers.raspi.ws281x.data.*
import com.poterion.monitor.notifiers.raspi.ws281x.deepCopy
import com.poterion.monitor.notifiers.raspi.ws281x.toColor
import com.poterion.monitor.notifiers.raspi.ws281x.toLightColor
...
class ConfigWindowController : BluetoothRaspiListener {
	companion object {
		internal fun getRoot(config: RaspiWS281xConfig, controller: RaspiWS281xNotifier): Parent =
	...
	private var config: RaspiWS281xConfig? = null
	private var controller: RaspiWS281xNotifier? = null
	...
	private fun saveConfig() {
		...
		config?.items = treeConfigs.root.children
				...
				.map { (node, children) ->
					RaspiWS281xItemConfig(node.title,
