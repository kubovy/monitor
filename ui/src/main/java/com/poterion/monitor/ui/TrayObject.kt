...
object TrayObject {
	...
	fun initialize(controller: ControllerInterface) {
	...
	private fun createMenu(controller: ControllerInterface) {
		try {
	...
	private fun Menu.createServicesMenuItems(controller: ControllerInterface) {
		controller.serviceControllers.sortedBy { it.config.order }.forEach { service ->
			add(service.menuItem.toMenu(controller, service.config))
		}
		if (controller.serviceControllers.isNotEmpty()) add(Separator())
	}

	private fun Menu.createNotifiersMenuItems(controller: ControllerInterface) {
		controller.notifierControllers.sortedBy { it.config.name }.forEach { notifier ->
			add(notifier.menuItem.toMenu(controller, notifier.config))
		}
		if (controller.notifierControllers.isNotEmpty()) add(Separator())
	}
	...
	private fun Item.toMenu(controller: ControllerInterface, moduleConfig: ModuleConfig): Entry? {
		return if (title != null && sub != null) { // Menu
			...
			MenuItem(title, { Platform.runLater { action?.invoke() } }).also {
				it.enabled = enabled
			}
